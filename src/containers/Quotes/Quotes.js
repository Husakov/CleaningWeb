...
class Quotes extends Component {
    ...
    async createQuoteToggle(quote = new QuoteModel(this.props.user.selectedCompany.payment_settings.deposit_amount,this.props.user.selectedCompany.payment_settings.tax_percent)) {
        if (quote.id !== -1) {
            const newQuote = await quotesapi.getQuote(this.props.user.selectedCompany.id, quote.quote_id);
            this.props.deleteErrors();
            this.setState({ isOpen: !this.state.isOpen, quote: newQuote });
    ...
    render() {
        return (
            <Container fluid>
                <Row>
                    <Col md={2} className="customer-actions pr-0">
                        <p className="text-center m-0 py-1 border-bottom">Actions</p>
                        <Button color="link" className="p-2 border-bottom w-100"
                            onClick={() => this.createQuoteToggle()}>Create
                            new quote</Button>
                    </Col>
